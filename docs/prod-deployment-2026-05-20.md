# ASTRAPHE Production Deployment Guide
...
| ------------------- | --------------------------------------------------- | --------------------------- |
| `astrapheai.com`     | **Existing waitlist/marketing site — do not touch** | Wherever it currently lives |
| `app.astrapheai.com` | SvelteKit web app (SPA)                             | Firebase Hosting            |
| `api.astrapheai.com` | FastAPI backend                                     | GCP Cloud Run               |


**Why this split:**

- `astrapheai.com` already serves your waitlist — adding subdomains has zero impact on it. DNS records for `app.` and `api.` are additive only.
...
- Webhook endpoints live at `api.astrapheai.com/v1/sync/<provider>/webhook` — no separate `webhooks.` subdomain needed.

**Optional future subdomains to consider:**

- `status.astrapheai.com` — uptime status page (Instatus or Better Uptime, both have free tiers)
- `docs.astrapheai.com` — redirect to `api.astrapheai.com/docs` (FastAPI auto-generates interactive Swagger docs)
...
The current `ALLOWED_ORIGINS` list references `https://astraphe.app`, which is not your production domain. Replace it:
...
`_ALLOWED_RETURN_HOSTS` and the `.endswith()` check both reference `astraphe.app`. Update to `astrapheai.com`:
...
2. Verify: `GET https://api.astrapheai.com/v1/admin/users` with your Bearer token should return a user list, not a `403`.
...
4. Sender email: something like `noreply@astrapheai.com` (requires a verified Resend domain)
...
```powershell
gcloud config set project astraphe-ai-coach
...
gcloud projects add-iam-policy-binding astraphe-ai-coach \
...
steps:
  - name: 'gcr.io/cloud-builders/docker'
    args:
      - build
      - -t
      - gcr.io/$PROJECT_ID/astraphe-api:$COMMIT_SHA
      - -t
      - gcr.io/$PROJECT_ID/astraphe-api:latest
      - .
    dir: backend

  - name: 'gcr.io/cloud-builders/docker'
    args: [push, --all-tags, gcr.io/$PROJECT_ID/astraphe-api]
  ...
  - name: 'gcr.io/google.com/cloudsdktool/cloud-sdk'
    ...
    args:
      ...
      - deploy
      - astraphe-api
      - --image=gcr.io/$PROJECT_ID/astraphe-api:$COMMIT_SHA
      - --region=us-central1
...
```powershell
cd C:\Users\seanb\Documents\astraphe-ai-coach\backend
gcloud builds submit --config cloudbuild.yaml .
```

### 5f. Map the custom domain to Cloud Run

After the first successful deployment, map `api.astrapheai.com`:

```powershell
gcloud beta run domain-mappings create `
  --service=astraphe-api `
  --domain=api.astrapheai.com `
  --region=us-central1
```
...
| Concern                        | Service                                                                           |
| ------------------------------ | --------------------------------------------------------------------------------- |
| Web app at `app.astrapheai.com` | Firebase Hosting                                                                  |
| REST API, webhooks, AI coach   | GCP Cloud Run (`api.astrapheai.com`) — unchanged                                   |
...
1. [Firebase Console](https://console.firebase.google.com/) → **Add project** (e.g. `astraphe-prod`) or use an existing project.
2. **Link to GCP** (recommended): Project settings → Integrations → link to GCP project `astraphe-ai-coach` so Hosting, Cloud Run, and Secret Manager share one billing account.
3. In the same project, register apps you need:
  - **Web** — optional label for Hosting; not required for FCM.
  - **iOS** — bundle ID `com.astraphe.coach` → download `GoogleService-Info.plist` → `mobile/ios/App/App/` (see PUSH_NOTIFICATIONS.md).
  - **Android** — package `com.astraphe.coach` → `google-services.json` → `mobile/android/app/` when you ship Android.
...
| Variable                | Value                                                         |
| ----------------------- | ------------------------------------------------------------- |
| `VITE_API_URL`          | `https://api.astrapheai.com`                                   |
...
### 6d. Custom domain `app.astrapheai.com`

Firebase Console → Hosting → **Add custom domain** → `app.astrapheai.com`.

Firebase provides DNS records (often a CNAME to `your-project.web.app` plus a TXT for domain verification). Add them at the same DNS provider that manages `astrapheai.com` — **do not change** existing root/`www` waitlist records.
...
**CI (GitHub Actions):** Store `FIREBASE_SERVICE_ACCOUNT` JSON as a repo secret (Firebase Console → Project settings → Service accounts → Generate new private key for CI, or use Workload Identity Federation with GCP). Workflow steps: `pnpm build` in `mobile/` → `firebase deploy --only hosting --project astraphe-prod`.
...
Store builds talk to `https://api.astrapheai.com`; they do not need the Firebase Hosting URL unless you load the web UI from `app.astrapheai.com` inside the WebView.

---

## 7. DNS Configuration

`astrapheai.com` is already live and serving your waitlist — **do not touch the root domain or `www` records.** You only need to add two new subdomain records. Do this at whatever DNS provider currently manages `astrapheai.com` (check your registrar or look up the current nameservers).
...
**For `app.`:** Use only the records Firebase shows when you add `app.astrapheai.com`. Firebase provisions its own CDN and TLS for Hosting.

### Registrar vs Cloudflare DNS

Add the `app` and `api` records at whatever currently hosts DNS for `astrapheai.com`. You do **not** need to move the root domain or waitlist site. Existing `astrapheai.com` / `www` records stay unchanged.

### Verify propagation

```powershell
Resolve-DnsName api.astrapheai.com
Resolve-DnsName app.astrapheai.com
```
...
# GCP
GCP_PROJECT_ID=astraphe-ai-coach
GCP_BUCKET_NAME=astraphe-workout-files-bucket
...
1. Open your app → Redirect URIs → Add:
  ```
   https://api.astrapheai.com/v1/sync/oauth/whoop/callback
  ```
...
1. Update **Authorization Callback Domain** to: `api.astrapheai.com`
...
2. The full callback URL used in your code is:
  ```
   https://api.astrapheai.com/v1/sync/oauth/strava/callback
  ```
...
```
https://api.astrapheai.com/v1/sync/oauth/garmin/callback
```
...
Webhooks must be re-registered to point to the production API. Complete this **after** `api.astrapheai.com` is live and returning `200` on the health check.

### WHOOP Webhook

In the [WHOOP Developer Dashboard](https://developer-dashboard.whoop.com/) → your app → **Webhooks**:

1. Set the webhook URL to: `https://api.astrapheai.com/v1/sync/whoop/webhook`
...
**Production (register once):** With `api.astrapheai.com` live:
...
Set `APP_BASE_URL=https://api.astrapheai.com` in `.env` (or export it) before running the script so it registers the production callback. The script deletes stale subscriptions, registers the URL from `APP_BASE_URL`, and writes `STRAVA_WEBHOOK_SUBSCRIPTION_ID` into `backend/.env`.
...
- **Webhook URL:** `https://api.astrapheai.com/v1/sync/garmin/webhook`
...
2. **Site URL:** `https://app.astrapheai.com`
3. **Redirect URLs** — add all of the following:
  ```
   https://app.astrapheai.com/auth/callback
   https://app.astrapheai.com/auth/reset-password
   astraphe://auth/callback
   astraphe://auth/reset-password
   http://localhost:5173/auth/callback
   http://localhost:5173/auth/reset-password
  ```
   The `astraphe://` entries are for your Capacitor mobile app (the deep link scheme is set in `capacitor.config.ts` as `com.astraphe.coach` and the scheme `astraphe` is in your `MOBILE_DEEP_LINK_SCHEME` setting). The `redirectUrl.ts` utility already handles selecting the correct URL based on whether the app is running natively or in a web browser.
4. **Email Templates** — update the Confirm Signup and Reset Password templates to use `https://app.astrapheai.com` in any redirect links. Check for any hardcoded localhost URLs in the default templates.
...
```json
{ "status": "healthy", "service": "ASTRAPHE API", "redis": "connected" }
```
...
```powershell
curl.exe -I https://api.astrapheai.com/health
```
...
```powershell
curl.exe -I -H "Origin: https://app.astrapheai.com" https://api.astrapheai.com/health
```

Must see: `Access-Control-Allow-Origin: https://app.astrapheai.com`

### Step 4: Frontend loads cleanly

- Open `https://app.astrapheai.com` in a browser
- DevTools → Network tab: no requests should be hitting `localhost` or `127.0.0.1`
- DevTools → Console: confirm `[API] Initialized with API_URL: https://api.astrapheai.com` (logged by `api.ts` line 9)

### Step 5: Auth flow end-to-end

1. Sign up with a new test email address
2. Confirm the verification email arrives via Resend
3. Click the email link — must redirect to `https://app.astrapheai.com/auth/callback`
...
curl -H "Authorization: Bearer $JWT" https://api.astrapheai.com/v1/athlete/profile
```
...
for i in $(seq 1 110); do
  STATUS=$(curl -s -o /dev/null -w "%{http_code}" \
    ...
    https://api.astrapheai.com/v1/athlete/state)
  echo "$i: $STATUS"
done
...
Trigger a test event from each provider (WHOOP: log a short activity or edit a sleep by 1 minute — see [WHOOP webhooks testing](https://developer.whoop.com/docs/developing/webhooks/#webhooks-testing)). Then in GCP Console → Cloud Run → `astraphe-api` → Logs:
...
2. Tap "Connect WHOOP" → authorize in the WHOOP web view → confirm the callback lands on `https://api.astrapheai.com/v1/sync/oauth/whoop/callback` → app returns to Connections showing "Connected"
3. Repeat for Strava
...
```powershell
curl.exe -s -o NUL -w "%{http_code}" https://api.astrapheai.com/v1/debug/connection
...
- **Target:** `https://api.astrapheai.com/health`
- **Check period:** 1 minute
- **Alert:** if the check fails 2 consecutive times

**Frontend uptime:** GCP Monitoring → Uptime check on `https://app.astrapheai.com` (same pattern as API health in §13), or Firebase Hosting status in the console. Optional: Firebase Performance Monitoring for web vitals after you add the web SDK.
...
| Resource                       | URL                                                        |
| ------------------------------ | ---------------------------------------------------------- |
| Web app                        | `https://app.astrapheai.com`                                |
| API base                       | `https://api.astrapheai.com`                                |
| API interactive docs (Swagger) | `https://api.astrapheai.com/docs`                           |
| Health check                   | `https://api.astrapheai.com/health`                         |
| WHOOP OAuth authorize          | `https://api.astrapheai.com/v1/sync/oauth/whoop/authorize`  |
| WHOOP OAuth callback           | `https://api.astrapheai.com/v1/sync/oauth/whoop/callback`   |
| WHOOP webhook                  | `https://api.astrapheai.com/v1/sync/whoop/webhook`          |
| Strava OAuth authorize         | `https://api.astrapheai.com/v1/sync/oauth/strava/authorize` |
| Strava OAuth callback          | `https://api.astrapheai.com/v1/sync/oauth/strava/callback`  |
| Strava webhook                 | `https://api.astrapheai.com/v1/sync/strava/webhook`         |
| Garmin webhook                 | `https://api.astrapheai.com/v1/sync/garmin/webhook`         |
