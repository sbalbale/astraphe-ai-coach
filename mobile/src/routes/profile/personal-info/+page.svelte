    ...
    <Card>
      <div class="flex flex-col gap-4">
        ...
        <div>
          ...
          <select id="gender" bind:value={gender} class="w-full p-2.5 bg-glass2 border border-border rounded-lg text-sm text-text0 outline-none focus:border-blue appearance-none">
            <option>Male</option>
    ...
    <Card>
      ...
      <div class="flex gap-3">
        <div class="flex-1">
          ...
          <input id="weight" type="number" bind:value={weight} class="w-full p-2.5 bg-glass2 border border-border rounded-lg text-sm text-text0 font-mono outline-none focus:border-blue" />
        </div>
        <div class="flex-1">
          <label for="height" class="block text-[11px] text-text2 mb-1">Height (cm)</label>
          <input id="height" type="number" bind:value={height} class="w-full p-2.5 bg-glass2 border border-border rounded-lg text-sm text-text0 font-mono outline-none focus:border-blue" />
        </div>
      </div>
    </Card>

    <button type="submit" class="w-full py-3.5 bg-blue text-white font-semibold rounded-xl hover:bg-[#3d1ae6] transition-colors mt-2">
      Save Changes
    </button>
  </form>
</div>
